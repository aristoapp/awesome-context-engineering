...
| Personal / team scope | Partial: buckets separate personal, project, and topic contexts; [shared buckets](https://help.memoryplugin.com/features/shared-buckets) let users share a bucket with another person, set permissions, and see who added what; sharing a bucket is a Pro feature, though invitees can accept on any plan. There is no organization or workspace product. |
...
- Chat-history import is plan-gated: the Core plan caps searchable imported chats at 500 from a single account on one platform; unlimited, all-platform history needs the Pro plan.
