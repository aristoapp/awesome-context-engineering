...
| [hippocampus](https://github.com/sturlese/hippocampus) | Local-first Obsidian vault template maintained by Claude Code skills and session hooks, deliberately built without a retrieval index: inbox notes are consolidated into typed, wikilinked pages, and retrieval is a three-step read (working-memory cache, then master index, then the few relevant pages) rather than embedding search. Contradictions are flagged on both pages instead of overwritten, and a stdlib linter enforces link and index integrity. | Emerging Organize/Evolve candidate covering the consolidation stage without a vector store; activation is Claude Code skills and hooks rather than MCP, so cross-client reach is narrow, and scaling behaviour of index-only retrieval on large vaults, plus real usage evidence, still need evaluation before a full profile. |
