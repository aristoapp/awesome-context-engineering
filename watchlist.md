...
| [Undercroft](https://github.com/latticeworklabs-eng/undercroft) | Open-core CLAUDE.md operations contract that makes Claude Code the autonomous maintainer of a plain-Markdown personal wiki: three-layer raw/wiki/schema architecture with strict ownership boundaries, checklist-driven ingest with contradiction flagging instead of silent overwrites, and append-only index/changelog bookkeeping; Obsidian-compatible and portable to other agent CLIs via an AGENTS.md rename. | Emerging local-workspace candidate; newly published, and the free tier ships the schema plus the full ingest operation while query, lint, and working-memory operations are in a paid kit, so long-term maintenance behavior still needs source-backed evaluation before a full profile. |
