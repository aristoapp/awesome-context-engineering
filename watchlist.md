...
| [Agent QA](https://github.com/vostride/agent-qa) | Open-source QA agent that builds execution memory from product, suite, and test observations, reuses that context in future web and mobile test runs, and exposes dashboard, CLI, MCP, and portable skill surfaces. | Specialized workflow-memory candidate rather than a general second brain; its memory is scoped to test execution and healed steps, so broader knowledge capture, governance, and cross-workflow reuse are not established. |
