...
| [BundleDex](https://bundledex.net) | OKF bundle directory: 409 bundles, 3,889★, 177 OKF-conformant. OKF bundles are the format for AI-agent second brains — consumed by Claude Code agents for knowledge management. Includes MCP tools, API, llms.txt, and an AGENTS.md vs Cursor Rules vs OKF comparison guide. | Emerging directory/resource for discovering OKF bundles that serve as agent knowledge; adjacent to second-brain content but not itself a memory system. |
