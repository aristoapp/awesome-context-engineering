...
| [LocalKeep](https://github.com/zuneracodes/localkeep) | Local-first personal timeline for Claude: you curate titled, dated, editable memory banks that live on your machine (SQLite Hub UI); Claude recalls them via MCP instead of inventing uneditable cloud memory. | Emerging local-first Use/Govern candidate; human-authored banks + MCP recall are shipped (PyPI + official MCP registry), while Collect/Evolve automation is intentionally out of scope, so it is not yet a full evaluated profile. |
