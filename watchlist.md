...
| [Off Grid AI Desktop](https://github.com/off-grid-ai/off-grid-ai-desktop) | On-device macOS app for a personal second brain, split across two tiers. The AGPL-3.0 open core runs a bundled local LLM (llama.cpp) for chat and RAG search over your own data, plus MCP connectors and on-device image generation - all inference runs locally, with no account, no telemetry, and no cloud. A separate proprietary paid Pro package, not part of the AGPL source tree, adds the memory layer: screen capture to OCR and entities, persistent Day/Entities/Replay memory in local SQLite, reflection, meeting and voice transcription, and approval-gated actions with an audit log. | Emerging local-first candidate spanning Collect, Organize, and Use lifecycle stages for a personal second brain. Source and license boundary is explicit: the AGPL open core (model runner, gateway, MCP connectors, image generation) is verifiable from the public repo, while the screen capture, persistent Day/Entities/Replay memory, reflection, and approval-gated action features ship in a proprietary paid Pro package that is not in the AGPL tree. Not yet a full evaluated profile. |
