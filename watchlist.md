...
| [GotSaeng OS](https://github.com/wonkwonlee/gotsaeng-os) | Local-first Markdown and Obsidian context compiler that generates source-aware context packs, memory diffs, and review reports without cloud, telemetry, or LLM calls. | Shipped source-backed local-workspace candidate. Its AI use is currently a manual file handoff, with no MCP/API/SDK or automatic context-injection path, so it does not yet meet the Core activation bar. |
