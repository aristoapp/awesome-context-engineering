...
| [marm-memory](../solutions/marm-memory.md) | Yes, local Python package or Docker image with SQLite storage and HTTP/STDIO MCP | Not a hosted product | Local-first MCP memory with hybrid recall, staged compaction, and optional knowledge/code graphs | You operate the MCP server, capture workflow, optional dependencies, and any team sharing or permissions model. |
