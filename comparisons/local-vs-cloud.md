...
| [Orbismo](../solutions/orbismo.md) | No | Yes | Hosted typed knowledge graph plus a rules/conventions layer, reachable directly from Claude or ChatGPT chat without running any infrastructure | Hosted-only; no self-hosted or local path, and no built-in automatic ingestion connectors. |
