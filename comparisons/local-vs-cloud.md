...
| [taOSmd](../solutions/taosmd.md) | Yes, fully local/offline; SQLite store, ONNX/NPU embeddings, optional GPU | No hosted/managed option by design | Fully offline agent memory on small/low-end hardware with no cloud or API keys | You operate the local embedder and a local LLM (Ollama/rkllama); no managed convenience layer. |
