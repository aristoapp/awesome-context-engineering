...
| [Stigmergy](../solutions/stigmergy.md) | Yes, and the only model: a self-hosted service, Postgres + pgvector, an S3-compatible store, and a git repo you own | No hosted option; model calls go to the provider you configure | A team brain on infrastructure you control, with pages as Markdown in your own git repo | You operate the whole stack, including the Slack app and the scheduled jobs. |
