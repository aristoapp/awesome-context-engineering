...
| [GotSaeng OS](../solutions/gotsaeng-os.md) | Yes, local Markdown/Obsidian vault plus local CLI or desktop plugin | No built-in hosted path; any AI upload or sync is an external user choice | Deterministic, source-aware context packs with no runtime cloud or model dependency | No managed connectors, sync, semantic retrieval, direct agent activation, or team controls. |
