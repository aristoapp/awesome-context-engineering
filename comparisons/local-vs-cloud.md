...
| [MemoryPlugin](../solutions/memoryplugin.md) | No; the open-source local MCP server is a client to the hosted API, not local storage | Yes | One hosted memory across web chat tools (extension) and MCP clients without integration work | Hosted-only storage; portability path is memory export as CSV/JSON/text. |
