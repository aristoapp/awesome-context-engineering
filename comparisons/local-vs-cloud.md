...
| [GotSaeng OS](../solutions/gotsaeng-os.md) | Yes, local Markdown/Obsidian vault plus local CLI, local stdio MCP server, or desktop plugin | No built-in hosted path; any AI upload or sync is an external user choice | Deterministic, source-aware context packs with no runtime cloud or model dependency, reachable from local MCP clients | No managed connectors, sync, semantic retrieval, or team controls; MCP access is read/compile-only, not vault write-back. |
