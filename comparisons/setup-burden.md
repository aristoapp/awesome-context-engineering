...
| [GotSaeng OS](../solutions/gotsaeng-os.md) | Run the published CLI or MCP server via `npx`, or install the desktop Obsidian plugin; choose the vault/output/project, and review generated reports. | Easy to start, but users own source quality, backups/sync, and heuristic review. MCP access is read/compile-only, so users still choose what an agent sees. |
