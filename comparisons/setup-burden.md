...
| [Orbismo](../solutions/orbismo.md) | Add the Orbismo connector in Claude or ChatGPT and log in, then work through chat; optionally install a starter world template or mini app from the public [orbismo-playbook](https://github.com/orbismo/orbismo-playbook) repo. | Storage, indexing, embeddings/search infrastructure, and MCP server hosting. There is no built-in automatic ingestion connector, so getting data in beyond what the connected AI writes during chat is on the user. |
