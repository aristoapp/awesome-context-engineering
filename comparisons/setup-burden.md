...
| [marm-memory](../solutions/marm-memory.md) | Python package or Docker image, local SQLite data path, HTTP or STDIO MCP configuration, and optional embedding/concept-graph dependencies. | Core logging works without optional graph features, but capture quality depends on agent discipline and shared use needs one server process per SQLite database. |
