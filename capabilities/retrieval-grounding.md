...
| GotSaeng OS | Partial | Generated Markdown/JSON context packs, Obsidian Report Hub with source-note navigation and backlinks, and MCP `list_context_artifacts`/`read_context_artifact` so an MCP client can read compiled output directly, source-linked. | No semantic/vector search or RAG; retrieval is reading generated files by name, not a query interface. |
...
| Vestige | Built-in | `recall` folds similarity search, cross-memory reasoning, and contradiction detection with Reciprocal Rank Fusion; `backfill` adds a separate backward, entity-linked causal-recall path. | Grounding quality depends on the local embedding model and on whether the host agent records retrieved context; the causal-recall benchmark is maintainer-published (see the profile). |
