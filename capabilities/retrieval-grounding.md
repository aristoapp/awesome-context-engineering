...
| Data Olympus | Built-in | Full-text search filtered by status, type, tier, validity and in-force guidance, an abstention mode, and an optional local-embedding hybrid ranking extra. | Default retrieval is lexical; the project's own benchmarks show weak recall on token-disjoint paraphrases. |
