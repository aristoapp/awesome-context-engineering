...
| GotSaeng OS | Partial | MCP tool calls (`compile_context_pack`, `read_context_artifact`, `prepare_ai_handoff`) are logged, source-linked, and return `sha256`/byte metadata for every artifact, so what a client read is inspectable after the fact. | No acted-on proof: GotSaeng OS cannot confirm a downstream AI actually used what it read, only that the read happened. |
...
- [Hjarni solution profile](../solutions/hjarni.md)
