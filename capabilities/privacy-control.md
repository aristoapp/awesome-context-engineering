...
| taOSmd | Built-in | Fully local SQLite store with no cloud egress or API keys; the append-only transcript provides a verifiable audit trail, user activity/browsing capture is opt-in, and a human review/correction queue controls durable memory. | User owns operations and backups; correction-supersede applies to the typed-KG layer, not raw vector chunks. |
