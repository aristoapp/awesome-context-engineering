...
| taOSmd | Built-in | Fully local SQLite store with no cloud egress or API keys; the append-only transcript provides a verifiable audit trail, user activity/browsing capture is opt-in, and a human review/correction queue controls durable memory. | User owns operations and backups; correction-supersede spans both the typed-KG and vector layers (zero-loss: the raw row and an append-only archive entry are retained). |
