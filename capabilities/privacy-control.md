...
| GotSaeng OS | Built-in for local storage | Local Markdown/JSON files only; no telemetry, hidden network calls, credential collection, cloud sync, or LLM API calls; the MCP server treats vault content as untrusted data and caps read sizes. | Privacy of any later AI upload, model provider, or sync choice a user makes with the exported files is outside GotSaeng OS. |
