...
| taOSmd | Local/offline | SQLite-backed local store with ONNX MiniLM embeddings on CPU or RK3588 NPU, optional GPU, and a small local LLM via Ollama/rkllama. No cloud and no API keys. | No hosted/managed option by design; the user operates the local embedder and local LLM. |
