...
| GotSaeng OS | Local-first | CLI, local stdio MCP server (vault/output roots fixed via launch flags, no network listener), or a desktop-only Obsidian plugin — all read a local vault and write to a local output folder. | No hosted path; any AI upload or sync of the exported files is an external user choice. |
...
| Vestige | Local-first | Single prebuilt Rust binary installed via npm; runs as a local stdio MCP server with local SQLite (optional SQLCipher) and local embeddings; no cloud, Docker, or API key. | Single-machine and local-only; no hosted or team deployment, and a one-time embedding-model download is needed. |
