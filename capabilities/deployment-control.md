...
| Data Olympus | Local/self-hosted | Git-backed Markdown directory plus a local or self-hosted MCP/REST service; PyPI package through `uvx` or `uv tool install`, and a Docker Compose path. | No managed hosted product; operators run the repository, service, index and backups. |
