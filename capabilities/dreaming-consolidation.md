...
| taOSmd | Built-in | Contradiction detection and invalidation (corrected singular facts supersede across both the typed-KG and vector layers; recall returns only the active fact), near-duplicate detection (Jaccard), retention scoring with promotion gates, and reflect/consolidation, with a human review queue before changes become durable. | Supersede excludes stale entries from recall but retains the raw row and an append-only archive entry, so it is zero-loss rather than a hard delete. |
