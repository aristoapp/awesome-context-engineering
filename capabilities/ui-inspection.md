...
| taOSmd | Partial | A human review/correction queue plus CLI, Python SDK, and local REST access let users inspect and correct memory; the append-only transcript is auditable. | No GUI; review and correction happen through the CLI, SDK, API, and queue rather than a hosted dashboard. |
