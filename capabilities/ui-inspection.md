...
| taOSmd | Partial | A minimal read-only local web UI (`taosmd serve`) for searching memory, viewing the pending-review queue, and checking health, plus a human review/correction queue and CLI, Python SDK, and local REST access; the append-only transcript is auditable. | The web UI is minimal and read-only (no destructive actions, not a full visual knowledge UI); correction happens through the CLI, SDK, API, and queue rather than a hosted dashboard. |
