...
| taOSmd | Built-in + Integration | `pip install taosmd`, a local ONNX embedder, and a small local LLM via Ollama/rkllama; then run the `taosmd` CLI, `taosmd serve` (local REST plus minimal read-only web UI), or `taosmd mcp`. | The pip package, CLI, and one-line bootstrap (Ollama plus model downloads) all complete a clean-machine run end-to-end on x86 Linux and the aarch64 Orange Pi NPU; the user operates the local models. |
