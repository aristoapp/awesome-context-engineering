...
| taOSmd | Built-in + Integration | `pip install taosmd`, a local ONNX embedder, and a small local LLM via Ollama/rkllama; then run the `taosmd` CLI, `taosmd serve` (local REST), or `taosmd mcp`. | The pip package and CLI are verified on a clean env; the one-line bootstrap (Ollama plus model downloads) is still being validated on clean machines, and the user operates the local models. |
