...
| Data Olympus | Built-in + Integration | `uvx` or `uv tool install`, a git-initialised knowledge bundle, the MCP server process, a setup wizard that registers supported agents, and `kb enforce` hooks. | Python 3.13+ and `uv` required; the operator keeps running the repository, service, agent wiring and backups, and no measured setup duration is published. |
...
| Vestige | Integration | One npm install of the prebuilt binary plus an MCP config entry; first run downloads an embedding model once. Intel Mac needs a Homebrew ONNX Runtime path. | Useful operation depends on the local model, memory-scope design, and how the agent-memory protocol is wired. |
