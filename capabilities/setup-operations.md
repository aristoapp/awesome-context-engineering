...
| GotSaeng OS | Built-in | Run the CLI or MCP server via `npx` from published npm packages, or install the desktop Obsidian plugin from the community registry; choose the vault/output/project. | Users still own vault hygiene, backups/sync, source quality, and report review. |
