...
| taOSmd | Built-in + API | Append-only, read-only Archive of both sides of a session (user and agent messages, tool calls/results, decisions, errors), optional opt-in user activity/browsing capture, a Python `ingest()` path, and typed loaders. | Capture is centered on agent-session transcripts and explicit ingestion; it is not a broad OAuth connector layer for SaaS sources. |
