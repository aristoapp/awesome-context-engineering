...
| GotSaeng OS | Built-in for local Markdown | CLI, MCP `compile_context_pack`, or the desktop Obsidian plugin scan a user-selected vault, parsing YAML frontmatter, explicit markers, task lists, and common section patterns. | No SaaS connectors, browser capture, or background collector; input is limited to local Markdown the user already has. |
